...
def cat_date_time():
    ...
    current_time = datetime.now(tz=pytz.timezone("Africa/Johannesburg"))

    return current_time


def combine_date_and_time(date, time) -> datetime:
...
def convert_12_hour_time_to_24_hour_time(time_to_convert) -> str:
    ...
    """
    if isinstance(time_to_convert, (datetime, datetime.time)):
...
def convert_24_hour_time_to_12_hour_time(time_to_convert) -> str:
    ...
    for source_fmt, dest_fmt in zip(["%H:%M:%S", "%H:%M"], ["%I:%M:%S %p", "%I:%M %p"]):
        try:
            ...
            parsed_time = datetime.strptime(time_to_convert, source_fmt)
...
def convert_minutes_in_human_readable_format(minutes: float) -> str:
    ...
    parts = []
    if days:
        days_display = ngettext("%(count)d day", "%(count)d days", days) % {
            "count": days
        }
        parts.append(days_display)

    if hours:
        hours_display = ngettext("%(count)d hour", "%(count)d hours", hours) % {
            "count": hours
        }
        parts.append(hours_display)

    if minutes:
        minutes_display = ngettext("%(count)d minute", "%(count)d minutes", minutes) % {
            "count": minutes
        }
    ...
    if len(parts) == 1:
        ...
        return _("{first_part} and {second_part}").format(
            first_part=parts[0], second_part=parts[1]
        )
    elif len(parts) == 3:
        return _("{days}, {hours} and {minutes}").format(
...
def convert_str_to_date(date_str: str) -> datetime.date:
    ...
    raise ValueError(
        f"Invalid date format for '{date_str}'. Supported formats are YYYY-MM-DD and YYYY/MM/DD."
    )
...
def get_ar_end_time(start_time, duration) -> datetime.time:
    ...
    if not isinstance(start_time, (datetime.time, str)):
        raise TypeError(
            "start_time must be a datetime.time object or a string in 'HH:MM:SS' format."
        )

    if not isinstance(duration, (datetime.timedelta, int, float)):
        raise TypeError(
            "duration must be either a datetime.timedelta or a numeric type representing minutes."
        )
    ...
    dt_start_time = datetime.combine(datetime.today(), start_time)
...
def time_difference(time1, time2):
    # If inputs are datetime.time objects, convert them to datetime objects for the same day
    if isinstance(time1, datetime.time) and isinstance(time2, datetime.time):
        ...
        raise ValueError(
            "Both inputs should be of the same type, either datetime.time or datetime"
        )
...
def get_date_variables_from_string_date(string_date):
    """Returns a date object, today's date and a formatted date"""
    date_obj = datetime.strptime(string_date, "%d-%m-%Y").date()
    today_date = date.today()
    formatted_date = date_obj.strftime("%a, %B %d, %Y")
    return date_obj, today_date, formatted_date


def get_current_time():
    current_time = cat_date_time().time()
    # Format the time without microseconds
    formatted_time = current_time.strftime("%H:%M")
    return formatted_time


def get_formatted_time_now_cal():  # Get the current date and time
    current_time = cat_date_time()
    print(current_time)

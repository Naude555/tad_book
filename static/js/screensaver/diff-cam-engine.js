var DiffCamEngine = (function () {
	...
	function init(options) {
		...
		scoreThreshold = options.scoreThreshold || 30000;
		...
		// console.log('pixelDiffThreshold',pixelDiffThreshold);
		// console.log('includeMotionPixels',includeMotionPixels);
		// console.log('motionContext',motionContext);
	...
	function startComplete() {
		...
		// stopInterval = setTimeout(stop,2000);
	}
	...
	function capture() {
		...
		curImgData = captureImageData.data;


		if (isReadyToDiff) {
			// var diff = processDiff(diffImageData);
			var diff = processDifference(prevImgData, curImgData);
		...
		// diffContext.globalCompositeOperation = 'source-atop';
		// diffContext.drawImage(video, 0, 0, diffWidth, diffHeight);
		prevImgData = captureImageData.data;
		isReadyToDiff = true;
	}

	function processDifference(prevRGB, curRGB) {
		var rgba = curRGB.map(function (item, index) {
			if ((index + 1) % 4 === 0) {
				return item;
			} else {
				return Math.abs(item - prevRGB[index])
			}
		});
		...
		for (var i = 0; i < rgba.length; i += 4) {
			rgba[i] = Math.abs(curRGB[i] - prevRGB[i]);
